...
public class RegisterActivity extends AppCompatActivity {
                ...
                switch (id) {
                    case R.id.verifyBtn:
                        if (Utility.isNetworkAvailable(getApplicationContext())) {
                            if (isMandatoryField()) {
                                if (verifyBtn.getText().toString().equalsIgnoreCase("Login")) {
                                    createVerifyJsonObject();
                                } else if (verifyBtn.getText().toString().equalsIgnoreCase("register")) {
    ...
    private void parseVerifyResponse(JSONObject response) {
        try {
            ...
            if (status == 1) {
                verifyBtn.setText("Register");
                employeeIdEditText.setEnabled(false);
                employeePasswordEditText.setTextColor(getResources().getColor(R.color.gray));
                employeeIdEditText.setTextColor(getResources().getColor(R.color.gray));
                employeePasswordEditText.setEnabled(false);
            } else {
                Utility.displayMessage(this, "Wrong Credentials");
                verifyBtn.setText("Login");
                employeePasswordEditText.setTextColor(getResources().getColor(R.color.black));
                employeeIdEditText.setTextColor(getResources().getColor(R.color.black));
                employeeIdEditText.setEnabled(true);
                employeePasswordEditText.setEnabled(true);
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
